...
function setupInputScrollHandlers() {
    ...
    const allInputs = document.querySelectorAll('#plan-name-input, #goal-description-input, #days-count-input, .gpt-input, .gpt-textarea');
        ...
        const handleFocus = (e) => {
            ...
            requestAnimationFrame(() => {
                setTimeout(() => {
                    ...
                    // Рассчитываем позицию с учетом отступа 40px
                    const targetPosition = scrollTop + rect.top - 40;
                    
                    // Используем scrollTo для прокрутки
                    window.scrollTo({
...
function setupScreenHandlers() {
    ...
    if (continueBtn1) {
        continueBtn1.addEventListener('click', async () => {
            ...
            if (!planName) {
                await window.customModal?.error('Введите название плана') || alert('Введите название плана');
                return;
            }
    ...
    if (continueBtn2) {
        continueBtn2.addEventListener('click', async () => {
            const description = goalDescriptionInput.value.trim();
            if (!description) {
                await window.customModal?.error('Опишите вашу цель') || alert('Опишите вашу цель');
                return;
            }
