...
function updatePriorityHeader(tasks) {
    ...
    if (tasks.length === 0) {
        ...
        priorityText.textContent = 'Приоритет 1';
        return;
    }
    ...
    priorityText.textContent = `Приоритет ${maxPriority}`;
}
...
async function loadTasksForDate(date) {
    ...
    try {
        ...
        if (tasks.length === 0) {
            taskList.innerHTML = '<div class="empty-state">Нет задач на этот день</div>';
            return;
        }
        ...
        taskList.innerHTML = '<div class="error-state">Ошибка загрузки задач</div>';
    }
}
...
function createTaskCard(task) {
    ...
    const priorityIcon = task.priority ? 
        `<img src="${getPriorityIcon(task.priority)}" alt="Приоритет ${task.priority}" class="task-priority-icon" width="16" height="16">` : 
        '';
    ...
    card.innerHTML = `
        ...
        <div class="task-menu-wrapper">
            ...
            <div class="task-menu-dropdown" data-task-id="${task.id}">
                <button class="task-menu-item" data-action="rename">Переименовать</button>
                <button class="task-menu-item" data-action="edit">Редактировать</button>
                <button class="task-menu-item" data-action="delete">Удалить</button>
            </div>
        </div>
`;
...
function setupEventHandlers() {
    ...
    if (burgerMenu && sidebarOverlay) {
        ...
        // Закрытие при клике на overlay (но не на кнопки)
    ...
    function handleSidebarAction(action) {
        switch(action) {
            ...
            case 'info':
                ...
                // Здесь можно добавить открытие информации
                break;
            case 'support':
